"""Polymarket Soccer — Arbitraje vs Bookmakers Bot."""
from __future__ import annotations
import sys
from datetime import datetime
from strategy import find_arb_signals
from portfolio import SoccerPortfolio


def run() -> None:
    print(f"\n{'='*55}")
    print(f"  Soccer Arb Bot  —  {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"{'='*55}\n")

    portfolio = SoccerPortfolio()
    print(f"[bot] Equity: ${portfolio.total_equity():,.2f}  |  Posiciones: {len(portfolio.open_positions)}")

    signals = find_arb_signals(top_n=5)
